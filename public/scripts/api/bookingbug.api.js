...
// BOOKINGBUG LOGIN
// =================================
const getBookingbugAuthToken = () => {
  return new Promise( (resolve, reject) => {
    $http("GET", "api/auth").then( (result) => {
      resolve(result);
    }, err => {
      reject(err);
    })
...
const loginComplete = () => {
	return new Promise((resolve, reject) => {
    getBookingbugAuthToken().then( (result) => {
      const data = JSON.parse(result);
      const authToken = data.authToken;
			requestHeaders.push(["App-Id", data.appId]);
			requestHeaders.push(["App-Key", data.appKey]);
      localStorage.clear();
...
const bookingbugAddItem_POST = (data) => {
	// ===========================================================================
	// NOTE: I have to get the auth-token by logging into starfox.studio and
	// creating a booking (the auth-token that I get when logging in via this
	// application is no good for making bookings, I have no idea why)
	// ===========================================================================
